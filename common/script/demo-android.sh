...
# NOTE : This script should be used for running ANDROID SAMPLES of App Configuration service.
#!/bin/bash
set -e
...
printf "2. Cleanup - Delete all the existing entries of collection, feature flags, properties and segments from your instance\n\n"
...
addSegments()
{
	...
	segmentStatus=$(curl -s --write-out 'HTTPSTATUS:%{http_code}'  -X POST $segmentUpdateURL -H "Authorization: Bearer $access_token" -H "Content-Type: application/json" --data '{"segment_id": "'"${segmentId}"'", "name": "IBM employees","description": "All employees of IBM","tags": "automation, demo,alliance","rules" :  [{"attribute_name" : "email","operator": "endsWith","values": [ "ibm.com"]}]}' )
	...
	days=$(($(date +'%s * 1000 + %-N / 1000000')))
	segmentId="segment_${days}"
	segmentStatus=$(curl -s --write-out 'HTTPSTATUS:%{http_code}'  -X POST $segmentUpdateURL -H "Authorization: Bearer $access_token" -H "Content-Type: application/json" --data '{"segment_id": "'"${segmentId}"'", "name": "Software testers","description": "Group of individuals involved in testing the functionality of a product.","tags": "automation, demo,test" ,"rules" : [{"attribute_name" : "email","operator": "is","values": [ "alice@bluecharge.com","bob@bluecharge.com"]}]}' )
	HTTP_BODY=$(echo $segmentStatus | sed -e 's/HTTPSTATUS\:.*//g' | jq .)
	HTTP_STATUS=$(echo $segmentStatus | tr -d '\n' | sed -e 's/.*HTTPSTATUS://')
	if [ $HTTP_STATUS != 201 ]
	then
...
addCollection()
{
	collectionStatus=$(curl -s --write-out 'HTTPSTATUS:%{http_code}'  -X POST $collectionURL -H "Authorization: Bearer $access_token" -H "Content-Type: application/json" --data '{"name" : "Blue charge android","collection_id": "blue-charge-android","description": "Demo App","deleted" : "false","created_mode": "Dashboard","tags": "demo"}' )
...
addEnvironment()
{
	environmentStatus=$(curl -s --write-out 'HTTPSTATUS:%{http_code}'  -X POST $environmentURL -H "Authorization: Bearer $access_token" -H "Content-Type: application/json" --data '{"name" : "'"${environmentName}"'","environment_id": "'"${environmentId}"'","description": "application environment","tags": "demo","color_code":"#FF5733"}' )
...
addFeature()
{
	...
	featureStatus=$(curl -s --write-out 'HTTPSTATUS:%{http_code}'  -X POST $featureUpdateURL -H "Authorization: Bearer $access_token" -H "Content-Type: application/json" --data '{"name": "Floating Profile","feature_id": "floating-profile","description": "Change from toolbar profile icon to  new floating profile icon on home page ","enabled_value": true,"type": "BOOLEAN","disabled_value": false,"tags": "demo","collections": [{"collection_id": "blue-charge-android"}],"segment_rules":[],"enabled": false}' )
	...
	featureStatus=$(curl -s --write-out 'HTTPSTATUS:%{http_code}'  -X POST $featureUpdateURL -H "Authorization: Bearer $access_token" -H "Content-Type: application/json" --data '{"name": "Flight Booking","feature_id": "flight-booking","description": "New major functionality for introducing Flight bookings","enabled_value": false,"type": "BOOLEAN","disabled_value": false,"tags": "demo","segment_rules": [{"rules": [{"segments": ["'"${testersSegmentId}"'"]}],"value": true,"order": "1"}],"collections": [{"collection_id": "blue-charge-android"}],"enabled": false}' )
	HTTP_BODY=$(echo $featureStatus | sed -e 's/HTTPSTATUS\:.*//g' | jq .)
	HTTP_STATUS=$(echo $featureStatus | tr -d '\n' | sed -e 's/.*HTTPSTATUS://')
	if [ $HTTP_STATUS != 201 ]
	then
