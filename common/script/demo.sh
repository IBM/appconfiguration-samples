...
{
	importStatus=$(curl -s --write-out 'HTTPSTATUS:%{http_code}'  -X POST $importURL -H "Authorization: Bearer $access_token" -H "Content-Type: application/json" --data @./configurations.json $importURL )
